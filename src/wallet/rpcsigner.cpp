...
UniValue signersend(const JSONRPCRequest& request)
{
    ...
    // TODO: pass options into FundTransaction as individual params
    ...
    if (complete) {
        ...
        UniValue err;
        const TransactionError error = pwallet->chain().broadcastTransaction(tx, txid, err_string, false);
        if (TransactionError::OK != error) {
            throw err_string;
        }
        result.pushKV("txid", txid.GetHex());
    } else {
